...
fn okHit(t: f32, n: vec3<f32>, mat: Material, shaderId: u32) -> HitInfo {
    return HitInfo(true, t, normalize(n), mat, shaderId, 0u, 1.0);
}
...
fn trace(ray0: Ray) -> vec3<f32> {
    ...
    for (var depth: i32 = 0; depth < MAX_BOUNCES; depth = depth + 1) {
        ...
        var mode: u32 = hRel.shaderId;
        if hRel.shaderId == 0u {
            mode = cam.matteMode;
        } else if hRel.shaderId == 2u {
            mode = cam.sphereMode;
        }
...
fn fsMain(@location(0) img: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    let texel = textureLoad(my_texture, vec2<u32>(U, V), 0);

    let outRGB = pow(max(texel.rgb, vec3<f32>(0.0)), vec3<f32>(1.0 / cam.gamma));
