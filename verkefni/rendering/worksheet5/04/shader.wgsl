...
@group(0) @binding(7) var<storage, read> MAT_IDX : MatIndexBuf;
@group(0) @binding(8) var<storage, read> MATERIALS : MaterialsBuf;
...
fn intersectTriangleFace(ray: Ray, faceIdx: u32, tmin: f32, tmax: f32) -> HitInfo {
    ...
    return okHit(t, n, mat, 0u, vec2<f32>(beta, gamma));
}
...
fn shade_once(ray: Ray, hit: HitInfo) -> vec3<f32> {
    ...
    if occluded_from(P, L.wi, L.dist) { return hit.mat.emission; }

    let nl = max(0.0, dot(N, L.wi));
    var Lo = hit.mat.emission + hit.mat.diffuse * L.Li * nl;
