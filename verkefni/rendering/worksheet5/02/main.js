async function init() {
    ...
    function writeUniforms() {
        ...
        f32.set([U[0], U[1], U[2], texDiv], 4);     // U (w used for magnification divisor)
        f32.set([V[0], V[1], V[2], 0.0], 8);        // V
        f32.set([W[0], W[1], W[2], 0.0], 12);       // W
        f32.set([aspect, zoom, gamma, invW], 16);   // _pad0 holds invWidth
        u32[20] = (addrMode >>> 0);
        u32[21] = (filterMode >>> 0);
    ...
    // vertices come as vec4 padded (Float32Array)
    const vertF32 = di.vertices;

    // indices are packed [i0,i1,i2,matIdx, ...]; convert to pure triangles
    const faceCount = di.mat_indices.length; // one per triangle
    ...
    // NEW: Upload normals buffer
    const normalsF32 = di.normals;  // Get normals from OBJ parser
    ...
    // tiny uniform for mesh info (faceCount); 16B for alignment
    ...
    // NOTE: bindings must match shader:
    // 0: Camera uniform, 1: texture_2d, 2: jitters buffer,
    // 3: positions (storage), 4: indices (storage), 5: mesh info (uniform)
    const bindGroup = device.createBindGroup({
        ...
        entries: [
            ...
            { binding: 3, resource: { buffer: vertBuffer } },   // <— NEW
            { binding: 4, resource: { buffer: indexBuffer } },  // <— NEW
            { binding: 5, resource: { buffer: meshInfoBuf } },  // <— NEW
        ],
    });
    ...
    // Optional: WGSL compile diagnostics (helpful!)
