...
fn intersect_scene_bsp(ray_in: Ray) -> HitInfo {
    ...
    var stack_level: array<u32, 40>;
    var stack_ptr = 0u;
    
    for (var i = 0u; i <= MAX_LEVEL; i = i + 1u) {
        let node = BSP_TREE.data[node_idx];
        ...
        if node_axis_leaf == BSP_LEAF {
            ...
            for (var j = 0u; j < node_count; j = j + 1u) {
                ...
                let h = intersectTriangleFace(ray, tri_idx);
                if h.hit && h.t < best.t {
                    best = h;
                    ray.tmax = h.t;
                }
            ...
            if stack_ptr == 0u {
                ...
                ray.tmin = stack_tmin[stack_ptr];
                ray.tmax = stack_tmax[stack_ptr];
