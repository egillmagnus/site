function loadTurtleModel(gl, url, callback) {
    fetch(url)
        .then(response => response.text())
        .then(data => {
            const objData = parseOBJ(data);
            const buffers = initTurtleBuffers(gl, objData);
...
function initTurtleBuffers(gl, objData) {
    ...
    const singleColor = [85 / 255, 107 / 255, 47 / 255, 1.0];
