...
function render() {
    ...
    mv1 = mult(mv1, translate(armLength / 2, 0.0, 0.0));
    mv1 = mult(mv1, scalem(armLength, armWidth, armWidth));
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays(gl.TRIANGLES, 0, numVertices);

    var mv2 = mult(mv, rotateZ(hourAngle));
    mv2 = mult(mv2, translate(armLength, 0.0, 0.0));
    mv2 = mult(mv2, rotateZ(minuteAngle));
    mv2 = mult(mv2, translate(armLength / 2, 0.0, 0.0));
    mv2 = mult(mv2, scalem(armLength, armWidth * 0.8, armWidth * 0.8));
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv2));
    gl.drawArrays(gl.TRIANGLES, 0, numVertices);

    var mv3 = mult(mv, rotateZ(hourAngle));
    mv3 = mult(mv3, translate(armLength, 0.0, 0.0));
    mv3 = mult(mv3, rotateZ(minuteAngle));
    mv3 = mult(mv3, translate(armLength, 0.0, 0.0));
